/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║  WORLD OF DRIVER — SIGNATURE PREMIUM  v2.0                  ║
...
const WOD_SOUND = (() => {
  ...
  return {
    // Ouverture app — accord doré ascendant
    open() {
      tone(220,'sine',0.12,0,0.25); tone(330,'sine',0.10,0.12,0.25);
      tone(440,'sine',0.14,0.22,0.35); tone(550,'sine',0.08,0.30,0.5);
      tone(660,'triangle',0.06,0.42,0.6);
    },
    // FAB principal — son distinctif
    fab() {
      tone(740,'sine',0.12,0,0.14); tone(987,'sine',0.08,0.06,0.18);
      tone(1109,'triangle',0.05,0.14,0.22);
    },
    // Clic standard
    click() {
      tone(880,'sine',0.07,0,0.08); tone(1100,'sine',0.03,0.02,0.06);
    },
    // Validation
    success() {
      tone(523,'sine',0.09,0,0.16); tone(659,'sine',0.09,0.08,0.18); tone(784,'sine',0.11,0.16,0.32);
    },
    // Dhikr tap
    dhikr() {
      tone(1047,'sine',0.07,0,0.11); tone(1319,'sine',0.04,0.04,0.09);
    },
    // Dhikr ×33 célébration
    dhikrComplete() {
      [523,659,784,1047,880,1175,1568].forEach((f,i) => tone(f,'sine',0.10,i*0.11,0.22));
    },
    // Objectif
    goal() {
      tone(392,'sine',0.10,0,0.14); tone(523,'sine',0.10,0.10,0.18);
      tone(659,'sine',0.12,0.20,0.22); tone(784,'sine',0.08,0.30,0.45);
    },
    // Erreur
    error() {
      tone(200,'sawtooth',0.08,0,0.11); tone(160,'sawtooth',0.06,0.10,0.11);
    },
  };
})();
...
const WOD_IMG = {
  ...
  // SVG de remplacement si aucune image trouvée
...
function injectSignatureStyles() {
  ...
  style.textContent = `
    ...
    .notif-hd-logo {
      ...
      background: #0a0f1a; margin-right: 7px; flex-shrink: 0;
    ...
    .dhikr-celebration-logo {
      position: relative; z-index: 1; width: 150px; height: 150px;
...
function patchHeader() {
  // Remplacer l'img existante dans hd-left par notre logo identifié
  const hdLeft = document.querySelector('.hd-left');
  if (!hdLeft) return;

  // Trouver l'img dans hd-left et lui donner notre id
  const existingImg = hdLeft.querySelector('img');
  if (existingImg) existingImg.id = 'hd-wod-logo-small';

  // Trouver le hd-titles et injecter une img logo
  ...
  if (hdTitles && !document.getElementById('hd-wod-logo')) {
    ...
    logoImg.style.cssText = 'height:22px;width:auto;object-fit:contain;filter:drop-shadow(0 0 8px rgba(212,168,67,0.8));display:block;border-radius:0;';
...
// ══════════════════════════════════════════════════════════════
//  ANIMATED COUNTERS — observer sur les valeurs clés
// ══════════════════════════════════════════════════════════════
...
const DHIKR = {
  ...
  _injectWidget() {
    ...
    // Insérer après la ia-card, ou en fin de scroll
  ...
  _injectCelebration() {
    ...
    div.innerHTML = `
      ...
      <img class="dhikr-celebration-logo" id="dhikr-celebration-logo"
        data-wod-logo="1" alt="WOD" src="./icon-192.png">
...
async function initWODSignature() {
  ...
  // Re-apply logo after dynamic content
